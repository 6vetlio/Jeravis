...
def speak(engine: Kokoro, text: str, speaking_event=None, interrupt_event=None):
    clean = text
...
def listen_voice(recognizer, microphone, input_queue: queue.Queue, speaking_event):
    while True:
        ...
        try:
            ...
            if text:
                ...
                if speaking_event.is_set() and should_interrupt(text):
                    input_queue.put(("interrupt", text))
                    continue
...
def listen_text(input_queue: queue.Queue):
    while True:
        try:
            text = input()
            if text.strip():
                input_queue.put(("text", text.strip()))
        except EOFError:
            break
...
def response_worker(pending_queue: queue.Queue, history: list, memory: dict, engine: Kokoro, speaking_event, interrupt_event, state: dict):
    while True:
        query = pending_queue.get()
        if query is None:
        ...
        response = handle_direct_query(query, memory)
        if response is None:
            response = ask_ollama(query, history, memory, interrupt_event)
        ...
        process_response(response, memory, lambda t: speak(engine, t, speaking_event, interrupt_event), interrupt_event)
        state["processing"] = False
...
def main():
    ...
    if not ollama_ready:
        print("[!] Continuing without a live Ollama connection. The first request will retry automatically.")
    else:
        threading.Thread(target=warm_ollama_model, daemon=True).start()

    print("[TTS] Loading Kokoro voice engine...")
    _dir = os.path.dirname(os.path.abspath(__file__))
    engine = Kokoro(os.path.join(_dir, "kokoro-v1.0.onnx"), os.path.join(_dir, "voices-v1.0.bin"))
    ...
    state = {"active": False, "processing": False}

    print(f"\n[Ready] Say or type '{WAKE_WORD.upper()}' to activate.")
    print("[Tip]   You can also just type your message directly and press Enter.")
    speak(engine, "Jarvis online. Ready when you are.", speaking_event, interrupt_event)

    voice_thread = threading.Thread(target=listen_voice, args=(recognizer, microphone, input_queue, speaking_event), daemon=True)
    voice_thread.start()

    text_thread = threading.Thread(target=listen_text, args=(input_queue,), daemon=True)
    text_thread.start()

    worker_thread = threading.Thread(
        target=response_worker,
        args=(pending_queue, history, memory, engine, speaking_event, interrupt_event, state),
        daemon=True,
    )
    worker_thread.start()
    ...
    while True:
        try:
            source, text = input_queue.get(timeout=1)
        except queue.Empty:
            continue
        ...
        if source == "interrupt":
            ...
            if follow_up_query:
                state["active"] = True
                pending_queue.put(follow_up_query)
            continue

        if contains_wake_word(text) and not state["active"]:
            state["active"] = True
            query = extract_query_after_wake_word(text)
            if not query:
                if not state["processing"]:
                    speak(engine, "Yes?", speaking_event, interrupt_event)
                continue
        ...
        if normalized_query in ("goodbye", "go to sleep", "shut down", "exit"):
            ...
            speak(engine, "Going offline. Call me when you need me.", speaking_event)
            break
