        ...
        if filename.endswith('.py') and not filename.startswith('__'):
            ...
            try:
                ...
                if spec and spec.loader:
                    ...
                    if hasattr(module, 'NAME') and hasattr(module, 'TRIGGERS') and hasattr(module, 'execute'):
                        ...
                        print(f"[Plugins] Loaded: {module.NAME}")
            except Exception as e:
                print(f"[Plugins] Failed to load {filename}: {e}")

def check_plugins(query: str, memory: dict) -> str | None:
    """Check if any plugin triggers match the query."""
    ...
    for plugin in LOADED_PLUGINS:
        for trigger in plugin['triggers']:
            if trigger.lower() in query_lower:
                try:
                    result = plugin['execute'](query, memory)
                    if result:
                        return result
                except Exception as e:
...
def handle_direct_query(query: str, memory: dict) -> str | None:
    ...
    # Try plugins
    plugin_response = check_plugins(query, memory)
